...
serverResponse
  .then() // Pass the correct callback here
  .catch(); // Pass the correct error handling here
...
function responseHandler(resolve, reject) {
  const serverIsUp = Math.random() > 0.5; //this is a trick to get a 50% chance that the server is broken
  ...
  setTimeout(
    () => {
      if (serverIsUp) resolve(); // resolve withe the correct data
      else reject(); // reject with the correct error
    },
